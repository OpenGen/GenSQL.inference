(ns inferenceql.inference.gpm.crosscat-test
  (:require [inferenceql.inference.gpm.crosscat :as xcat]
            [clojure.test :refer [deftest is]]
            [inferenceql.inference.gpm :as gpm]))

;;; Building a two view XCat model with "color" and "height" in view-1 and "flip" in view-2.
...
(deftest simulate
  (let [row (gpm/simulate xcat-model [:color :height :flip] {})]
    ;; Simply checking that we generated all columns without error.
    (is (= (keys row) [:color :height :flip]))))
