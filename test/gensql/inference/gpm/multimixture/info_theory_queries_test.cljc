(ns gensql.inference.gpm.multimixture.info-theory-queries-test
  (:require #?(:clj [clojure.test :as test :refer [deftest testing is]]
            ...
            [gensql.inference.approximate :as approximate]
            [gensql.inference.gpm :as gpm]
            [gensql.inference.gpm.multimixture.search :as search]
...
#?(:clj (deftest simulate-from-MI-model
          ...
          (testing "(smoke) simulate n complete rows and save them as vl-json"
            (let [all-variables (keys (:vars multi-mixture))
                  samples (take sampled-points-for-plot (repeatedly #(gpm/simulate
                                                                       gpm-mmix
                                                                       all-variables
                                                                       {})))]
